...
- **LoCoMo 事实底盘管线已建**：BM25 + HyDE 扩查询检索、批量作答判分、mem0 × 0.9 及格线对照（`server/eval-locomo.ts`）。

## Roadmap

- [ ] LoCoMo 事实底盘基线验收（设计债务⑨）：管线就绪，待全量跑分——目标 mem0 等主流基线的 90%+
- [ ] 设计债务⑪：合规声明文本起草（not-a-medical-device 等三件）
- [ ] 评测系统「衔枝」独立化：冲突测试集已落地，待扩场景类型学与独立盲评流程
- [ ] embedding 向量预筛替换字符重合近似（防线在判定层，向量只是加速器；接口位置已留注）
